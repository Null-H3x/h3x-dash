...
CRED_TYPES = (
    ...
    'unix_hash',         # Linux /etc/shadow crypt() hash ($1$/$5$/$6$/$y$/...)
...
# Types that carry a crackable hash, used by stats() / UI hash counts.
HASH_TYPES = ('ntlm_hash', 'unix_hash')
...
class CredentialStore:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            # Tolerate a malformed/foreign schema (e.g. a bare list) without
            # crashing — keep only well-formed dict entries so one bad record
            # can't take down the whole Credentials page.
            if not isinstance(creds, dict):
                creds = {}
            self._creds = {k: v for k, v in creds.items() if isinstance(v, dict)}
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            'with_hash':     sum(1 for c in creds if c.get('type') in HASH_TYPES),
            'by_type':       by_type,
            'by_source':     by_source,
        }
...
# Hash id covers md5crypt($1$), bcrypt($2a/2b/2y$), sha256($5$), sha512($6$),
# yescrypt($y$/$gy$), scrypt($7$) — i.e. any "$<id>$..." crypt string. Username
# class allows uppercase, digits and dots (real service accounts e.g. GitLab).
    ...
    for m in _SHADOW_RE.finditer(output):
        ...
        creds.append({
            ...
            'tags':        ['hashcat:' + ('1800' if hash_str.startswith('$6$')
                                            ...
                                            else '3200' if hash_str.startswith(('$2a$', '$2b$', '$2y$'))
                                            else 'unknown')],
        })
    return creds
