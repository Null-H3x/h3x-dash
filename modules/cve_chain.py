...
# Modern Linux/Unix cmd exploits default to reverse_bash. Legacy lab images
# (Metasploitable 2/3, Linux 2.6) often ship dash/sh or bash compiled without
# /dev/tcp — reverse_bash fails with "No such file or directory" on /dev/tcp.
# distcc_exec only accepts generic/perl/ruby/bash per MSF Compat — NOT
# reverse_netcat_gaping (MSF rejects it as "payload not valid").
LEGACY_LINUX_CMD_PAYLOAD = 'cmd/unix/reverse_perl'

# If reverse callbacks are flaky (distcc timing), bind payloads are the lab fallback.
MSF_MODULE_PAYLOAD_FALLBACK: dict[str, str] = {
    'exploit/unix/misc/distcc_exec':              'cmd/unix/bind_perl',
    'exploit/unix/irc/unreal_ircd_3281_backdoor': 'cmd/unix/bind_perl',
    'exploit/multi/samba/usermap_script':         'cmd/unix/bind_perl',
}
...
def _legacy_linux_no_devtcp(host_os: str) -> bool:
    """True when the target likely cannot use bash /dev/tcp reverse shells."""
    osl = (host_os or '').lower()
    return any(m in osl for m in (
        'linux 2.', '2.6', 'metasploitable',
        'ubuntu 6', 'ubuntu 8', 'ubuntu 9', 'ubuntu 10',
        'debian gnu/linux 4', 'debian gnu/linux 5',
    ))
...
MSF_MODULE_PAYLOAD_OVERRIDE: dict[str, str] = {
    ...
    # IRC + distcc + Samba — MT2 command-exec classics. reverse_perl is in the
    # distcc_exec RequiredCmd compat list; reverse_bash /dev/tcp fails on MT2.
    'exploit/unix/irc/unreal_ircd_3281_backdoor':      LEGACY_LINUX_CMD_PAYLOAD,
    'exploit/unix/misc/distcc_exec':                   LEGACY_LINUX_CMD_PAYLOAD,
    'exploit/multi/samba/usermap_script':              LEGACY_LINUX_CMD_PAYLOAD,
...
def smart_payload(msf_module: str, host_os: str = '', host_arch: str = '') -> str:
    ...
    if msf_module in MSF_MODULE_PAYLOAD_OVERRIDE:
        payload = MSF_MODULE_PAYLOAD_OVERRIDE[msf_module]
        # Overrides like redis/web still default to reverse_bash — downgrade on
        # legacy lab Linux where /dev/tcp is unavailable (Metasploitable 2/3).
        if (payload == 'cmd/unix/reverse_bash'
                and _legacy_linux_no_devtcp(host_os)):
            return LEGACY_LINUX_CMD_PAYLOAD
        return payload
    ...
    if 'linux' in host_os_lower:
        ...
        if platform in ('unix', 'multi') and _legacy_linux_no_devtcp(host_os):
            return LEGACY_LINUX_CMD_PAYLOAD
    ...
    platform = _module_platform(msf_module)
    payload  = MSF_PLATFORM_PAYLOAD.get(platform, MSF_PLATFORM_PAYLOAD['multi'])
    if (platform in ('unix', 'multi')
            and payload == 'cmd/unix/reverse_bash'
            and _legacy_linux_no_devtcp(host_os)):
        return LEGACY_LINUX_CMD_PAYLOAD
    return payload
    ...
    def suggest(self, host: dict, ports: list,
        ...
        if enum_findings:
            for f in enum_findings:
                ...
                # Metasploitable classics — enum often surfaces these by name
                if 'distcc' in title or 'distccd' in title:
                    confirmed_modules.add('exploit/unix/misc/distcc_exec')
                if 'unrealircd' in title or 'unreal ircd' in title:
                    confirmed_modules.add('exploit/unix/irc/unreal_ircd_3281_backdoor')
                if 'vsftpd' in title and '2.3.4' in title:
                    confirmed_modules.add('exploit/unix/ftp/vsftpd_234_backdoor')
                if 'java rmi' in title or ('rmi' in title and 'java' in title):
                    confirmed_modules.add('exploit/multi/misc/java_rmi_server')
            ...
            for key in keys:
                for cve, msf_module, desc, severity in CVE_MAP.get(key, []):
                    ...
                    suggestions.append({
                        ...
                        'msf_payload_fallback': MSF_MODULE_PAYLOAD_FALLBACK.get(
                            msf_module or '', ''),
                        'description':    desc,
                        'severity':       severity,
