...
# ── 5. Driveby profile includes MT2 lab ports ─────────────────────────────────
driveby = PORT_PROFILES.get('driveby', '')
if isinstance(driveby, tuple):
    driveby = ''.join(driveby)
for mt2_port in ('1099', '3632', '6667'):
    if mt2_port not in str(driveby):
        fail(f"driveby profile missing MT2 port {mt2_port}: {driveby}")
if not any('driveby profile missing' in f for f in FAIL):
    ok(f"driveby profile includes MT2 ports 1099/3632/6667")

# ── 6. UI blurbs for port profiles ───────────────────────────────────────────
