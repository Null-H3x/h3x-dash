...
REPO_ROOT = AUDIT_DIR.parent   # audits live in audit/; app at repo root
...
AUDITS = [
    ...
    'audit_extensions.py',
    'audit_ops_log.py',
    'audit_housekeeping.py',
    'audit_callback.py',
    'audit_handoff.py',
    'audit_check_verdict.py',
    'audit_launch_profiles.py',
    ...
    'audit_exploit.py',
    'audit_shell.py',
    'audit_shell_chain.py',
    'audit_validate.py',
    'audit_enum.py',
    'audit_install.py',
    'audit_session_tool.py',
    'audit_payload_sources.py',
    'audit_credentials.py',
    'audit_loot.py',
    'audit_report.py',
    'audit_msf_session.py',
    'audit_msf_runner.py',
]


def _run(script: str) -> tuple[int, str]:
    path = AUDIT_DIR / script
    if not path.exists():
...
def main() -> int:
    ...
    vc = subprocess.run(
        [sys.executable, str(REPO_ROOT / 'validate_chain.py'), '--offline'],
        cwd=str(REPO_ROOT),
        capture_output=True,
        text=True,
    )
