...
# Console text may report opened before msfrpcd lists it — surface as
# unconfirmed, not a false success with phantom session IDs.
if (result.get('session_unconfirmed') is True
        and result.get('session_opened') is False
        and not result.get('exploit_failed')):
    ok("Session-opened-but-list-lagged: session_unconfirmed=True, "
       "session_opened=False (no phantom session IDs)")
else:
    fail(f"List-lag handling wrong: session_opened={result.get('session_opened')}, "
         f"session_unconfirmed={result.get('session_unconfirmed')}, "
...
if 'd.session_unconfirmed' in exploit_html and 'd.session_expected' in exploit_html:
    ok("Frontend distinguishes confirmed, unconfirmed, and non-session modules")
else:
    fail("Frontend missing session_unconfirmed / session_expected handling")
...
joined = '\n'.join(w)
if ('jobs -K' in joined
        and 'use exploit/multi/handler' in joined
        and 'run -j' in joined
        and 'set DisablePayloadHandler true' in joined
        and 'set WfsDelay 10' in joined):
    ok("distcc reverse_perl uses jobs -K + handler-first console sequence")
else:
