...
class MockClient:
    def __init__(self, sessions):
        self._sessions = {str(k): v for k, v in sessions.items()}
        self.sessions = MockSessions(self._sessions)

    def call(self, method, args=None):
        args = args if args is not None else []
        if method == 'session.list':
            return {
                sid: {'type': s.stype, 'target_host': '10.0.0.5',
                      'username': 'tester', 'platform': 'linux',
                      'arch': 'x64', 'info': 'mock', 'tunnel_local': ''}
        ...
        if method == 'session.shell_read':
            return {'data': sess.read()}
        if method == 'session.shell_write':
            sess.write(args[1])
            return {}
        if method == 'session.meterpreter_read':
            return {'data': sess.read()}
        if method == 'session.meterpreter_write':
            sess.write(args[1])
            return {}
        if method == 'session.meterpreter_run_single':
            sess._read_count = 0
            if getattr(sess, 'run_output', ''):
                sess.canned_output = sess.run_output
            sess.write(args[1])
            return {}
...
if ('_scheduleHandoff' in shell_script and '_drainHandoffQueue' in shell_script and
        'duplicate Meterpreter' in shell_script):
    ok("Auto-handoff serializes queue and dedups duplicate Meterpreter sessions")
else:
    fail("Auto-handoff queue / primary-session selection missing")

# Profile-driven handoff: cmd shells now get a handoff path too (not skipped)
if ('_cmdShellHandoff' in shell_script and '_meterpreterHandoff' in shell_script and
        'fetchHandoffProfile' in shell_script):
    ok("Handoff is profile-driven — cmd shells and Meterpreter each handled")
else:
    fail("Profile-driven handoff (cmd + meterpreter split) missing")

# Confirmation pipeline used before hardening (concern #2)
if ('/confirm' in shell_script and 'confirmSession' in shell_script):
    ok("Confirmation pipeline probes session liveness before hardening")
else:
    fail("Session confirmation pipeline not wired in shell handoff")

# Backend confirm/handoff methods are wired through to the engine
import inspect as _inspect
from modules.msf_engine import MsfEngine as _ME
if hasattr(_ME, 'confirm_session') and hasattr(_ME, 'resolve_session_handoff'):
    ok("MsfEngine exposes confirm_session + resolve_session_handoff")
else:
    fail("MsfEngine missing handoff/confirm methods")


# ── Robust dumb-shell confirmation (prime + sentinel + retries) ────────────────
from modules.msf_engine import MsfEngine as _ME2

class _SlowShellClient:
    """A reverse_perl-style shell: empty on the first N reads, then answers."""
    def __init__(self, wake_after=2):
    ...
    def call(self, method, args=None):
        args = args if args is not None else []
        if method == 'session.list':
            return {'1': {'type': 'shell', 'tunnel_local': '', 'info': ''}}
        if method == 'session.shell_write':
            self._writes += 1
            self._last = args[1]
            return {}
        if method == 'session.shell_read':
            if self._writes >= self._wake_after:
...
def _eng_with(client):
    e = _ME2()
    e._client = client
    e._connected = True
    return e

# Slow shell: a single quick probe would see empty, but retries confirm it.
eng_slow = _eng_with(_SlowShellClient(wake_after=2))
res = eng_slow.confirm_session('1', timeout=2, attempts=4)
if res.get('alive') and not res.get('dead'):
    ok("confirm_session: slow dumb shell confirmed via prime+sentinel retries "
       "(not abandoned on first empty read)")
else:
    fail(f"slow-shell confirm wrong: {res}")

# Dead shell: RPC raises 'does not exist' → dead=True (and the UI drops it).
class _DeadShellClient:
    def call(self, method, args=None):
        if method == 'session.list':
            return {'1': {'type': 'shell'}}
        if method in ('session.shell_write', 'session.shell_read'):
            raise Exception('Session ID (1) does not exist')
        return {}
eng_dead = _eng_with(_DeadShellClient())
res = eng_dead.confirm_session('1', timeout=1, attempts=2)
if res.get('dead') and not res.get('alive'):
    ok("confirm_session: genuinely dead session → dead=True (UI will drop it)")
else:
    fail(f"dead-shell confirm wrong: {res}")

# A quiet-but-not-dead shell must NOT be reported dead (UI keeps it interactive).
eng_quiet = _eng_with(_SlowShellClient(wake_after=999))   # never answers in window
res = eng_quiet.confirm_session('1', timeout=1, attempts=2)
if (not res.get('alive')) and (not res.get('dead')):
    ok("confirm_session: quiet shell → alive=False, dead=False (kept interactive, "
       "not abandoned)")
else:
    fail(f"quiet-shell confirm wrong: {res}")

# Frontend keeps non-dead cmd shells interactive (never 'skipping handoff' for them)
if ('_cmdShellHandoff' in shell_script and '_activateShellArea' in shell_script
        and 'NOT a dead shell' in shell_script or '_activateShellArea' in shell_script):
    ok("Frontend cmd-shell handoff activates the panel + only drops on d.dead")
else:
    fail("Frontend cmd-shell handoff robustness missing")

# Drain helper exists (clears buffer before the liveness probe)
import inspect as _isp
if 'def _drain_shell' in _isp.getsource(_ME2):
    ok("_drain_shell present — clears stale buffer before the sentinel probe")
else:
    fail("_drain_shell missing")

# ── Version label in sidebar ───────────────────────────────────────────────────
from config import H3xConfig
import re as _re
_base = Path('templates/base.html').read_text()
if '{{ app_version }}' in _base and 'logo-version' in _base:
    ok("Sidebar renders app version under the logo (logo-version + app_version)")
else:
    fail("Sidebar version label missing (logo-version / app_version binding)")

if _re.match(r'^\d+\.\d+\.\d+(\.\d+)?$', H3xConfig.VERSION):
    ok(f"H3xConfig.VERSION is version-shaped ({H3xConfig.VERSION})")
else:
    fail(f"H3xConfig.VERSION not version-shaped: {H3xConfig.VERSION!r}")

h3x_src = Path('h3x-dash.py').read_text()
if "'app_version'" in h3x_src and 'H3xConfig.VERSION' in h3x_src:
    ok("Context processor injects app_version from H3xConfig.VERSION (single source)")
else:
    fail("app_version not injected from H3xConfig.VERSION in context processor")
