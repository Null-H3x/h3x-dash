...
with tempfile.TemporaryDirectory() as tmp:
    ...
    listed = log.list_exploit_logs(rhost='192.168.56.101', limit=5)
    if listed and listed[0].get('module') and listed[0].get('has_txt'):
        ok('list_exploit_logs returns recent runs with txt companion')
    else:
        fail(f'list_exploit_logs unexpected: {listed}')

    art = log.read_exploit_artifact(ep.name)
    if art and art.get('type') == 'json' and art['data'].get('module'):
        ok('read_exploit_artifact loads json by basename')
    else:
        fail('read_exploit_artifact json load failed')

    if log.read_exploit_artifact('../../../etc/passwd') is None:
        ok('read_exploit_artifact blocks path traversal')
    else:
        fail('read_exploit_artifact allowed traversal')
