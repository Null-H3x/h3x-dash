...
COMPOSITE_OR_ALIAS = {
    ...
    'distcc_check', 'irc_check', 'rmi_check',
...
# ── 5c. MT2 classic ports fire distcc / IRC / RMI probes ────────────────────
mt2_ports = [3632, 6667, 1099]
mt2_fired = candidates_for(mt2_ports)
mt2_want = {'distcc_check', 'irc_check', 'rmi_check'}
if mt2_want.issubset(mt2_fired):
    ok(f"MT2 ports {mt2_ports} fire {sorted(mt2_want)}")
else:
    fail(f"MT2 ports missing enum tools — want {mt2_want}, got {mt2_fired & mt2_want}")

mt2_std = dispatch_for(mt2_ports, TIER_RECON)
if mt2_want.issubset(mt2_std):
    ok("MT2 enum probes run at Recon tier (fast, no deep sweep needed)")
else:
    fail(f"MT2 probes gated above Recon tier: missing {mt2_want - mt2_std}")
