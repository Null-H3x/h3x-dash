...
report_engine_inst = report_engine.ReportEngine(H3xConfig.REPORT_DIR)
...
def api_report_full():
    """Client-deliverable engagement report: cross-references scan, creds,
    exploit outcomes, validator verdicts and ATT&CK coverage into one HTML
    report. Secrets are masked unless include_secrets is set (internal copy)."""
    data = request.get_json(silent=True) or {}
    report_data = report_engine.assemble_from_live(
        scan_results  = scan_engine.get_results(),
        sessions      = msf_engine.list_sessions(),
        cred_store    = cred_store,
        msf_validator = msf_validator,
        ops_log       = ops_log,
        cve_chain     = cve_chain,
        engagement    = {
    ...
    result = report_engine_inst.generate(
        report_data,
        fmt=data.get('format', 'html'),
        include_secrets=bool(data.get('include_secrets')),
    )
    return jsonify(result)
...
_REPORT_NAME_RX = re.compile(r'^h3x-dash_(pentest_)?report_[0-9_]+\.(html|json)$')
